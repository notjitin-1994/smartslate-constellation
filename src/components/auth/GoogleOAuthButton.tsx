...
export function GoogleOAuthButton(): React.JSX.Element {
  ...
  async function onClick(): Promise<void> {
    try {
      ...
      alert('Google sign-in failed. Please try using email and password instead.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={loading}
      className="inline-flex w-full items-center justify-center gap-3 rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3 text-[11px] font-black text-white uppercase tracking-widest transition-all hover:bg-white/[0.05] disabled:opacity-50"
    >
